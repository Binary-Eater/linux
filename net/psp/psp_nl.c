...
int psp_nl_tx_assoc_doit(struct sk_buff *skb, struct genl_info *info)
{
	...
	fd = nla_get_u32(info->attrs[PSP_A_ASSOC_SOCK_FD]);
	err = psp_sock_assoc_set_tx(socket->sk, psd, &key, info->extack);
	if (err)
		goto err_free_msg;
		  ...
		  u32 portid, u32 seq, int flags)
{
	...
	if (nla_put_u32(rsp, PSP_A_STATS_DEV_ID, psd->id) ||
	    ...
	    nla_put_u64_64bit(rsp, PSP_A_STATS_RX_AUTH_FAIL,
			      stats.rx_auth_fail, PSP_A_STATS_PAD) ||
	    nla_put_u64_64bit(rsp, PSP_A_STATS_RX_ERROR,
			      stats.rx_error, PSP_A_STATS_PAD) ||
	    nla_put_u64_64bit(rsp, PSP_A_STATS_RX_BAD,
			      stats.rx_bad, PSP_A_STATS_PAD) ||
	    nla_put_u64_64bit(rsp, PSP_A_STATS_TX_PACKETS,
			      stats.tx_packets, PSP_A_STATS_PAD) ||
	    nla_put_u64_64bit(rsp, PSP_A_STATS_TX_BYTES,
			      stats.tx_bytes, PSP_A_STATS_PAD) ||
	    nla_put_u64_64bit(rsp, PSP_A_STATS_TX_ERROR,
			      stats.tx_error, PSP_A_STATS_PAD))
		goto err_cancel_msg;
